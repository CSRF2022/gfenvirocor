...
dat <- readRDS("data/yellowtail-comm-samples.rds") # cached data is missing lat, lon, and depth
...
dat <- left_join(dat, minor_areas) %>%
  mutate(longitude = ifelse(is.na(longitude), longitude2, longitude),
         atitude = ifelse(is.na(latitude), latitude2, latitude))
...
d %>%
  ...
  summarise(n = n(),
            ...
            depth_2.5 = wtd.quantile(best_depth, 0.025, na.rm = TRUE, weight = catch_weight),
            depth_97.5 = wtd.quantile(best_depth, 0.975, na.rm = TRUE, weight = catch_weight),
...
plot_maturity_map <- function(mat_data, d_all, months = c(1:12)){
  ...
  ggplot() +
    ...
    geom_jitter(aes(longitude, latitude, colour = Maturity, size = catch_weight),
                height = 0.1, width = 0.4, alpha = 0.9) +
...
ggsave(paste0("figs/", species, "-maturity-mapped.png"), width = 12, height = 10)
