...
fit_all_distribution_models <- function(species) {
  ...
  dens_model_name0 <- "dln-syn-only"
  dens_model_name <- "dln-syn-only"
...
# Start modeling ----
  ...
  d2 <- d1 %>%
    filter(year > 2001) %>%
    # # test pattern without extrapolating to years without ann survey proportions
...
if(maturity_possible) {
    ...
    if(nrow(which_surv)<2){
      mi <- sdmTMB(group_catch_est ~ 1 +
                   ...
                   spatiotemporal = as.list(mm[["spatiotemporal"]]),
                   time = "year",
                   family = mf$family,
...
A# Function for running species in parallel --------
#
