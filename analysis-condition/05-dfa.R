...
# m
  ...
  if(set_group == "immature") {
    ...
    f <- list.files(
      paste0("data-generated/density-index/dln-all-only-w-ann-prop/total"),
      pattern = ".rds",
      full.names = TRUE)
...
ggsave(paste0("figs/DFA/man/DFA-fits-",  trend_count,
...
if(trend_count>2){
  ggsave(paste0("figs/DFA/man/DFA-", trend_count, "trends-no-cov-",
                gsub(" ", "-", set_group), "-", model_name, "-",
                gsub(" ", "-", pro_covar1$Variable[1]), "-",
                gsub(" ", "-", pro_covar2$Variable[1]),
