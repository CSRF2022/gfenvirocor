...
d <- d %>% mutate(within_ci = ifelse(too_low == 0 & too_high == 0, TRUE, FALSE),
                  species = ifelse(
                    species == "Rougheye/Blackspotted Rockfish Complex",
                               "Rougheye/Blackspotted Rockfish", species)
                    )

ggplot(d) + geom_boxplot(aes(model_string, prop_diff))
# d1 <- filter(d, prop_ci_error < 0.05)
# d2 <- filter(d, prop_ci_error > 0.05)
d1 <- filter(d, prop_ci_error < 0)
d2 <- filter(d, prop_ci_error > 0)

hist(d2$prop_ci_error)
# absolute error
ggplot(d, aes(forcats::fct_rev(species), prop_diff, colour = model_string)) +
  ...
  geom_point(data = d2, aes(alpha = log(prop_ci_error)), colour = "black", size = 2) +
  # geom_point(data = d2, colour = "black", size = 2) +
  scale_alpha_continuous(range = c(0,1)) +
  guides(alpha = "none", colour=guide_legend(nrow=2)) +
  scale_color_discrete(
    labels = c("Only survey-years with morphological data from >= 18 fish & >= 6 sets",
  ...
  labs(x = "",
       ...
       # subtitle = "Inner circle shade indicates when the sum exceeds the CI on the overall index.",
       colour = ""
       # (solid black = > 5% of total biomass)."
...
ggsave("figs/model-comparison-abs-diff-logCIerror.png", height = 6, width = 7.5)
